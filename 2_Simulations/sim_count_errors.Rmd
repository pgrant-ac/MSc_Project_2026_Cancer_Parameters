...
# Function to format system time for human legibility 
...
```{r seq_depth_test, message=FALSE, warning=FALSE}

seq_model <- lm(seq_depth ~ pred_depth)
summary(seq_model)
  ...
  for (i in 1:(10*genes)){
    ...
    # While catching any warnings or errors
     fit_samples_corr <- withCallingHandlers(tryCatch({
       ...
       nb_gene(corr_vals, pred_depth[idx])
     },
     # Print out any error messages to highlight the problem
     error = function(e) {
       # Add one to the error count
...
Create updated plots with new colours at supervisors request.

```{r contours_updated_for_report, message=FALSE, warning=FALSE}
...
cols <- colorRampPalette(c("#FFEDA0", "#FD8D3C", "#B10026"))
