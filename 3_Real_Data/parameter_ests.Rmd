...
```
Now negative binomial estimates are finalised, can convert them into burst kinetics. A gene's expression can be described by it's burst frequency (b) (number of times transcription is turned on/bursts per mRNA lifespan), and burst size (k) (how many copies transcribed per burst). Because only relative sequencing depth is used these estimates are also relative and cannot be taken as exact values. Another custom function is used to convert these, using the equations:
...
pairwise_test <- function(df1, df2) {
  #' Function to calculate test statistics for k (burst size) and b (burst 
  #' frequency) between pre and post treatment for a cancer. Also calculates 
...
# Define row order by clustering on burst frequency (more changes)
...
*Figure 2a: Comparison of how many genes have significant burst size (k) change across cancers.*
...
*Figure 2b: Comparison of how many genes have significant burst frequency (b) change across cancers.*
